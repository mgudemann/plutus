...
class InputsFrom a where
  neededInputs :: a -> S.Set IdInput

instance inputsFromValue :: InputsFrom Value where
  neededInputs = cata algebra
    where
...
instance inputsFromObservation :: InputsFrom Observation where
  neededInputs = cata algebra
    where
      ...
      algebra (ValueGEF v1 v2) = foldMap neededInputs [v1, v2]
      algebra (ValueGTF v1 v2) = foldMap neededInputs [v1, v2]
      algebra (ValueLTF v1 v2) = foldMap neededInputs [v1, v2]
      algebra (ValueLEF v1 v2) = foldMap neededInputs [v1, v2]
      algebra (ValueEQF v1 v2) = foldMap neededInputs [v1, v2]
...
instance inputsFromContract :: InputsFrom Contract where
  neededInputs = cata algebra
    where
      ...
      algebra (CommitF _ _ _ value _ _ v1 v2) = neededInputs value <> v1 <> v2
      algebra (PayF _ _ _ value _ v1 v2) = neededInputs value <> v1 <> v2
      algebra (BothF v1 v2) = v1 <> v2
      algebra (ChoiceF o v1 v2) = neededInputs o <> v1 <> v2
      algebra (WhenF o _ v1 v2) = neededInputs o <> v1 <> v2
      algebra (WhileF o _ v1 v2) = neededInputs o <> v1 <> v2
      algebra (ScaleF v1 v2 v3 c) = foldMap neededInputs [v1, v2, v3] <> c
...
applyTransaction inputs sigs blockNum state contract value = case appResult of
  ...
  appResult = applyAnyInputs inputs sigs (neededInputs contract) blockNum expiredState reducedContract value emptyOutcome Nil
